 ...
 * 
 *Task A: - prints any new msg from queue 2
 *        - reads serial input from user
 *        - echo inp back to serial termnal
 *        - if "delay xxx" send xxx(number) to queue 1
 *Task B: - Updates t with any new value from queue 1
 *        - Blinks LED with t delay
 *        - Every time LED blinks 100 times, send "Blinkled"strig
 *          to Queue 2; also send number of times blinked 
 *          
...
void blinkLED(void *parameters) {
  ...
  while (1) {
    ...
    if (xQueueReceive(delay_queue, (void *)&led_delay, 0) == pdTRUE) {
      ...
      /* This part send msg back to taskCLI (ACK)*/
      // Best practice: use only one task to manage serial comms
      strcpy(msg.body, "Message from taskCLI received in taskBLINK");
      msg.count = 1;
